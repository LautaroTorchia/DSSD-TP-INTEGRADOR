...
export const useFurnitureStore = defineStore({
  ...
  actions: {
    async getCollectionFurniture(collectionId) {
      await this.getAll();
      const filteredFurniture = JSON.parse(
        localStorage.getItem("furniture"),
      ).furniture.filter((furniture) => furniture.coleccion == collectionId);
    ...
    async getFurnitureDetail(id) {
      const response = await fetchWrapper.get(
        `${baseUrl}/coleccion/muebles/${id}/`,
      );
      response.materiales = response.materiales
        .replace(/[\[\]']+/g, "")
        .split(",");
      return response;
    },
    ...
    async create(furniture) {
      const response = await sendFile.post(
        `${baseUrl}/coleccion/muebles/`,
        furniture,
      );
