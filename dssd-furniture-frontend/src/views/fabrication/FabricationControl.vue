...
const fetchFabricationReservations = async () => {
  try {
    const response = await fetchWrapper.get(
      `${import.meta.env.VITE_API_URL}/reservas/reservas-lugares-fabricacion/`,
    );
...
const filterFabricationReservations = async (fabrications) => {
  ...
  filteredFabrications.value = filteredFabrications.value.filter(
    async (reservation) => {
      return await fetchFabricationLocation(
        reservation,
        reservation.lugar_de_fabricacion,
      );
    },
  );
};
...
const renegociate = async () => {
  ...
  const controlarMaterialesTaskExists = bonitaTasks.some(
    (task) => task.displayName === "Controlar fabricación",
  );

  if (!controlarMaterialesTaskExists) {
    errorMessage.value =
      "Error: Todavía no ha pasado el tiempo estipulado para finalizar el control";
    return;
  }
    ...
    if (confirmed) {
      ...
      for (let i = 0; i < lotQuantity.value; i++) {
        const response = await fetchWrapper.post(
          `${baseUrl}/entregas/lotes-fabricados/`,
          data,
        );
      }
