  ...
  <div v-if="!loading">
    <div>
      ...
      <form @submit.prevent="renegotiate">
        <div class="mb-3">
          <label for="newReleaseDate" class="form-label"
            >Nueva Fecha de Lanzamiento:</label
          >
          <input
            type="date"
            id="newReleaseDate"
        ...
        <div class="d-flex justify-content-between mt-4">
          <button @click="renegotiate" class="btn btn-primary">
            Renegociar
          </button>
          <button @click="cancelCollection" class="btn btn-danger">
            Cancelar colección
          </button>
        </div>
      </form>
...
const renegotiate = async () => {
  ...
  try {
    ...
    deliveryOrders.value = await fetchWrapper.get(
      `${baseUrl}/entregas/ordenes/`,
    );
    deliveryOrders.value.map(
      async (order) =>
        await fetchWrapper.delete(`${baseUrl}/entregas/ordenes/${order.id}/`),
    );

    // get delivered material reservations
    const deliveredMaterialReservations = await fetchWrapper.get(
    ...
    const materialReservationsToDelete = allMaterialReservations.filter(
      (reservation) =>
        !deliveredMaterialReservations.some(
          (deliveredReservation) =>
            deliveredReservation.id_reserva === reservation.id,
        ),
    );

    // delete material reservations that were not delivered
    await Promise.all(
      materialReservationsToDelete.map((reservation) =>
        fetchWrapper.delete(
          `${baseUrl}/reservas/reservas-materiales/${reservation.id}/`,
        ),
      ),
    );
    ...
    await Promise.all(
      lugarFabricacionReservationsToDelete.map((reservation) =>
        fetchWrapper.delete(
          `${baseUrl}/reservas/reservas-lugares-fabricacion/${reservation.id}/`,
        ),
      ),
    );

    //now modify cantidad_materiales variable
    const materialReservationsToUpdate = allMaterialReservations.filter(
      (reservation) =>
        deliveredMaterialReservations.some(
          (deliveredReservation) =>
            deliveredReservation.id_reserva === reservation.id,
    ...
    if (behindSchedule === "true") {
      const cantidadMateriales = await getBonitaVariable(
        caseId.value,
        "cantidad_materiales",
      );
