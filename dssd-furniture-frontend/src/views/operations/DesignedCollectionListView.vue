...
onBeforeMount(async () => {
  ...
  const ordersPlaced = await fetchWrapper.get(
    `${baseUrl}/reservas/reservas-lugares-fabricacion/`,
  );
  designedCollections.value = collections.value.filter(
    (collection) => collection.designed && !collection.fabricated,
  );
  for (const collection of designedCollections.value) {
    collection.loading = true;
    try {
      const materialAmount = await getBonitaVariable(
        collection.caseId,
        "cantidad_materiales",
      );
      collection.cantidadMateriales = !!materialAmount;
      if (collection.cantidadMateriales) {
        const fabricationPlan = await getBonitaVariable(
          collection.caseId,
          "plan_de_fabricacion",
        );
        if (fabricationPlan === "") {
      ...
      if (collection.planDeFabricacion) {
        collection.orders_placed = !!ordersPlaced.find(
          (order) => order.coleccion == collection.id,
        );
      }
    } catch (error) {
      console.error(error);
