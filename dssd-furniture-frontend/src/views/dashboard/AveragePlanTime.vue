  ...
  <div :style="{ visibility: hiddenVisibility }">
    ...
    >
      <h4 class="text-center">
        Tiempo promedio de planeamiento: {{ averagePlanTime }} dias
      </h4>
      <canvas id="AveragePlanTimeChart" ref="chart"></canvas>
    </div>
  </div>
</template>
  ...
  return bonitaTasks
    .map((task) => {
      const collection = collectionList.find(
        (collection) => collection.caseId == task.caseId,
      );
      if (!collection) {
        return undefined;
      }
...
onMounted(async () => {
  ...
  const designTimeList = await getArchivedTasks(
    filteredCollections,
    "Diseñar coleccion",
  );
  const fabricationOrderList = await getArchivedTasks(
    filteredCollections,
    "Reservar espacio de fabricación",
  );

  const matchingItems = designTimeList
    .filter((designItem) => {
      return fabricationOrderList.some(
        (fabricationItem) => fabricationItem.caseId === designItem.caseId,
      );
    })
    .concat(
      fabricationOrderList.filter((fabricationItem) => {
        return designTimeList.some(
          (designItem) => designItem.caseId === fabricationItem.caseId,
        );
      }),
    );

  const dupPlanTimeList = matchingItems.map((item) => {
    const designTime = designTimeList.find(
      (designItem) => designItem.caseId === item.caseId,
    );
    const fabricationTime = fabricationOrderList.find(
      (fabricationItem) => fabricationItem.caseId === item.caseId,
    );
    return {
      name: item.name,
      caseId: item.caseId,
      planTime: Math.ceil(
        (new Date(fabricationTime.reached_state_date) -
          new Date(designTime.reached_state_date)) /
          (1000 * 60 * 60 * 24),
      ),
    };
  });

  const planTimeList = Array.from(
    new Set(dupPlanTimeList.map((planTime) => planTime.caseId)),
  ).map((caseId) => {
  ...
  averagePlanTime.value =
    planTimeList.reduce((sum, planTime) => sum + planTime.planTime, 0) /
    planTimeList.length;

  chart.value = new Chart(document.getElementById("AveragePlanTimeChart"), {
    type: "bar",
    data: {
