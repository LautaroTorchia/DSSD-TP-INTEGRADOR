...
const submitForm = () => {
  ...
  formData.value.materials = formData.value.materials.split(',').map(material => {
    const foundMaterial = materialsList.value.find(m => m.nombre === material)
...
const updateImageFile = (event) => {
  ...
  if (inputField.files.length > 0) {
    ...
    if (allowedExtensions.includes(fileExtension)) {
      ...
      displayErrorMessage(imageError, "Invalid file format. Please upload a .jpg, .jpeg, or .png file.")
    }
  }
}
...
const updateManufacturingPlanFile = (event) => {
  ...
  if (inputField.files.length > 0) {
    ...
    if (fileExtension === allowedExtension) {
      ...
      displayErrorMessage(manufacturingPlanError, "Invalid file format. Please upload a .pdf file.")
    }
  }
}
