        ...
        <div class="card shadow p-3 mb-5 bg-white rounded">
            ...
            <div class="card-body">
                <form @submit.prevent="submit">
                    <div class="row">
                        <div v-for="(order, index) in orders" :key="order.id" class="col-md-3">
                            <div class="card mb-3" :style="{ flex: cardFlex }">
                                <div class="card-body" style="text-align: center;">
                                    ...
                                    <select v-else v-model="order.lot" @change="updateLots(order)" class="form-select">
                                        <option v-for="lot in availableLots" :key="lot.id" :value="lot">
                                            Lote {{ lot.id }} Centro de distribucion {{ lot.distPlace }}
                                        </option>
                                    </select>
                                    <button @click="clearLot(order)" class="btn btn-danger">Clear</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
import { fetchWrapper, router, advanceNamedBonitaTaskWithCollection } from '@/helpers'
...
onMounted(async () => {
    ...
    const distPromise = fetchWrapper.get(`${baseUrl}/entregas/distribucion-de-lote/`)
    const distPlacesPromise = fetchWrapper.get(`${baseUrl}/entregas/lugares-de-distribucion/`)
    const finalSellersPromise = fetchWrapper.get(`${baseUrl}/entregas/vendedores-finales/`)
    ...
    orders.value.map((order) => {
        order.vendedor_final = finalSellers.find((seller) => seller.id == order.vendedor_final).nombre
    })
    const distPlaces = await distPlacesPromise
    const dist = await distPromise
    lots.value.forEach((lot) => {
        lot.distPlace = distPlaces.find((place) => place.id == dist.find((d) => d.lote == lot.id).distribucion).nombre
    })
    loading.value = false
})

const updateLots = (order) => {
    availableLots.value = availableLots.value.filter((lot) => lot.id != order.lot.id)
}
...
const submit = async () => {
    ...
    orders.value.forEach((order) => {
        ...
        promiseList.push(fetchWrapper.post(`${baseUrl}/entregas/orden-de-lote/`, data))
    })
