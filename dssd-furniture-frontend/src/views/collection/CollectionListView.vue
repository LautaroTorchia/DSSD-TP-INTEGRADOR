      ...
      <tbody>
        <template v-if="orderedCollections.length">
          <template v-for="(collection, index) in orderedCollections" :key="collection.id">
            <tr :style="{ 'background-color': index % 2 === 0 ? '#fff' : '#000', 'color': index % 2 === 0 ? '#000' : '#fff' }">
              ...
              <td>
                ...
                <button v-if="!collection.designed" @click="deleteCollection(collection.id)" class="btn btn-danger btn-sm ml-1">Borrar</button>
                <router-link v-if="!collection.designed" :to="{ name: 'collection-update', params: { collection: collection.id } }" class="btn btn-primary btn-sm ml-1">
                  <slot>Editar</slot>
                </router-link>
                <button v-if="!collection.designed" @click="finishCollection(collection)" class="btn btn-success btn-sm ml-1">Terminar</button>
              </td>
            </tr>
          </template>
        </template>
